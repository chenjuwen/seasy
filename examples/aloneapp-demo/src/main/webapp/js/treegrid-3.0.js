...
;(function($){
	...
	var TreeGrid = function(_target, options){
		...
		var show_count = settings.show_count || false;
		
		drowHeader = function(){
			s += "<tr class='header'>";
			s += "<td width='25' align='center'></td>"; 
		...
		drowRowData = function(_rows, _level, _pid){
			...
			for(var i=0; i<_rows.length; i++){
				...
				str += "<tr height='25' id='r" + trid + "' pid='" + ptrid + "' rowindex='' rowlevel='" + _level + "' style='display:" + dispaly + "'>";
		...
		init = function(){
			...
			_container.find("td.menuItem").hover(
				...
				function(){
					$(this).css("background-color", "#AED0EA");
				}
			);
			...
			_container.on("mousedown", "tr[pid]", function(e){
				...
				if(1 == _event.which){ 
					...
					if(menus && menus.length > 0){
						...
						$(menu).css("left", _event.pageX - 230);
						$(menu).css("top", _event.pageY - 60);
						$(menu).css("display", "");
					}
				}
			});
			...
			_container.on("click", "td.menuItem > a", function(){
				var code = $(this).parents("td[trid]:first").attr("code") || "";
				var handler = $(this).parents("td[trid]:first").attr("handler") || "";
				...
				if(trid && trid!=""){
					...
					if(rowdata){
						if(handler && handler != ""){
							$(this).prop("href", "javascript:" + handler + "('" + code + "','" + trid + "');");
						}else if(url && url!=""){
		...
		this.show = function(){
			...
			if(menus && menus.length > 0){
				...
				for(var i=0; i<menus.length; i++){
					...
					if(_menu.handler && _menu.handler !=""){
						s += "<tr><td class='menuItem' code=\"" + (_menu.code||"") + "\" trid=\"\" handler=\"" + _menu.handler + "\"><a href=\"javascript:void(0);\">" + (_menu.title||"") + "</a></td></tr>";
					}else{
						s += "<tr><td class='menuItem' code=\"" + (_menu.code||"") + "\" trid=\"\" url=\"" + (_menu.url||"javascript:void(0);") + "\"><a href=\"javascript:void(0);\" target='"+(_menu.target||"_blank") + "'>" + (_menu.title||"") + "</a></td></tr>";
					}
				}
				s += "</table><br>";
			}
